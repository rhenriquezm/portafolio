...
public class NivSegDaoImpl implements NivSegDao {
    ...
    public ArrayList<NivSeg> getAll() throws Exception {
        try {
            ...
            ArrayList<NivSeg> nseg = (ArrayList<NivSeg>) criteria.list();
    ...
    public boolean deleteById(short id) throws Exception {
        try {
            this.session = HibernateUtil.getSessionFactory().openSession();
            this.transaction = this.session.beginTransaction();
            NivSeg nsegu = (NivSeg) session.load(NivSeg.class, id);
            this.session.delete(nsegu);
            this.transaction.commit();
            return true;
        } catch (Exception e) {
            if (this.transaction != null) {
    ...
    public boolean update(NivSeg nsg) throws Exception {
        try {
            ...
            NivSeg nisg = (NivSeg) session.load(NivSeg.class, nsg.getIdNivSeg());
            nisg.setDescNivSeg(nsg.getDescNivSeg());
            session.update(nisg);
            this.transaction.commit();
            return true;
        } catch (Exception e) {
